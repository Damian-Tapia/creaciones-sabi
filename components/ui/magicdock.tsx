...
function DockItem({
  ...
  return (
    ...
    >
      <motion.div
        className={cn(
          "relative flex h-full w-full items-center justify-center rounded-full shadow-md transition-colors duration-300",
          getBorderStyles()
        )}
        initial={{}}
      >
        {item.image ? (
          <img
            src={item.image}
            alt={item.label}
            className="h-full w-full rounded-full object-cover object-center p-1 fill-white text-white"
          />
        ) : (
        ...
        <AnimatePresence>
          {hoveredIndex === item.id && (
            <motion.div
              ...
              className={cn(
                "absolute z-50 -translate-x-1/2 flex-col items-center justify-center rounded-md bg-transparent px-4 py-2 text-xs shadow-xl",
